...
| `max_actions_per_day`      | number  | Daily step cap -- documented; not enforced by a persisted counter here (disclosed limitation).                                                                                    |
...
| `consecutive_api_errors`   | number  | Counter toward `error_trip_threshold` (documented; not incremented automatically in this demo -- a real deployment increments it in the HTTP node's error branch).                |
