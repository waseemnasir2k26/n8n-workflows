...
| `n8n_base_url`               | string  | **This instance's own** scheme+host, e.g. `https://n8n.example.com` or `http://127.0.0.1:5678` -- no trailing slash. Read by `Load caps` (now the FIRST node after the triggers, ahead of `List workflows`) so the watchdog's self-referential API call never hardcodes a scheme/host. Falls back to `http://localhost:5678` in the expression itself if the column is empty/missing -- the repo copy ships with the column absent, which is the same as that default. |
...
error_trip_threshold 3 · n8n_base_url https://n8n.skynetjoe.com` (each deployed
instance sets `n8n_base_url` to its OWN scheme+host -- never copy another
instance's value).
